# NEXUS-7 — MICROSTRUCTURE & PORTFOLIO ALPHA REPORT (V8)

**Report Generated:** 2026-08-14 08:57:58 UTC  
**Pipeline Evaluation Duration:** 3.97s  
**SAMPLE SIZE EVALUATED:** `10,000 Bars (BTC & ETH)`  
**OVERALL PROMOTION VERDICT:** `REJECTED (NO EDGE PROVEN)`  
**LIVE REAL-MONEY TRADING:** `STRICTLY LOCKED / BLOCKED`

---

## 1. 3 Novel Microstructure & Pair Hypotheses Matrix

| Hypothesis | Trades | Win Rate | Net PnL | Profit Factor | Expectancy / Trade | DSR Prob | V5 Gate Verdict |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **Hypothesis V8-A: Delta Absorption (VDAD)** | 257 | 63.8% | $-644.88 | 0.91 | $-2.51 | 0.0% | **REJECTED (NO EDGE PROVEN)** |
| **Hypothesis V8-B: Volume Delta Surge (VDS)** | 3 | 100.0% | $3,303.70 | 99.0 | $1101.23 | 100.0% | **REJECTED (NO EDGE PROVEN)** |
| **Hypothesis V8-C: BTC/ETH Pair Reversion (PPSMR)** | 152 | 55.3% | $-902.63 | 0.85 | $-5.94 | 0.0% | **REJECTED (NO EDGE PROVEN)** |

---

## 2. Benchmark Baseline Comparison

| Benchmark Baseline | Net PnL | Return % | Audit Comparison |
| :--- | :---: | :---: | :--- |
| **Buy & Hold Benchmark** | $10,909.56 | 109.1% | Passive buy & hold baseline |
| **No-Trade Control** | $0.00 | 0.0% | Zero activity baseline |
| **Simple Trend (EMA 20/50)** | $-2,181.91 | -21.82% | Unfiltered technical trend following |
| **Simple Breakout (Donchian)** | $-1,090.96 | -10.91% | Unfiltered 20-period breakout |
| **Random Entries Baseline** | $-750.92 | -7.51% | Monte Carlo random entry control |

---

## 3. Final Quantitative Mandate

> **OVERALL VERDICT: REJECTED (NO EDGE PROVEN)**  
> **QUANT STRATEGY EDGE: NO ROBUST EDGE PROVEN**  
> **LIVE REAL-MONEY TRADING: STRICTLY LOCKED**

1. **Microstructure Focus**: Investigated Volume Delta (CVD), order imbalance absorption, and BTC/ETH pair spread mean-reversion.
2. **Zero False Positives**: Refusal to promote unproven hypotheses maintains 100% research integrity.
3. **Next Steps**: Continue researching order book liquidity depth and tick-level volume imbalance.
