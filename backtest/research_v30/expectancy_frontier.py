"""
NEXUS-7 Research V30 — Expectancy Frontier Module
Constructs Pareto Frontier showing Frequency vs Expectancy vs Profit Factor vs Drawdown vs Robustness.
Ranks candidate strategies using robustness-first scoring.
"""
...
def build_expectancy_frontier(
    ...
    """
    Ranks candidate strategies using robustness-first multi-factor scoring.
    ...
    for res in candidates_results:
        ...
        in_target_window = (0.75 <= tpd <= 1.50)
        ret_to_dd = (exp_usd / (mdd * 1000.0 + 1e-8)) if mdd > 0 else 0.0

        # Robustness-first multi-factor score
        score = (
    ...
    # Best frequency in target window [0.75, 1.50]
    ...
    # Best robust (highest ci_lower)
    sorted_robust = sorted(all_rows, key=lambda x: x["ci_lower"], reverse=True)
    best_robust = sorted_robust[0] if sorted_robust else None

    # Determine overall official verdict category
    if any(r["verdict"] == "ROBUST_EDGE_FOUND" for r in all_rows):
