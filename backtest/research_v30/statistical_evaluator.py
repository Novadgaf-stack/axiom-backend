"""
NEXUS-7 Research V30 — Statistical Evaluator Module
Computes true Profit Factor, Win Rate, Net Expectancy, Max Drawdown, Trades/Day,
1,000-iteration Bootstrap 95% CIs, economic significance metrics, sample size checks (>= 100 OOS trades),
and maps strategy candidates to official verdict categories.
"""
...
def compute_trade_statistics(
    ...
    # Longest losing streak calculation
    max_streak = 0
    current_streak = 0
    for p in pnls:
        if p < 0:
    ...
    in_target_freq = 0.75 <= trades_per_day <= 1.50
