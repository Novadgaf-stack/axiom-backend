"""
NEXUS-7 Research V30 — Robustness Testing Module
Provides neighboring parameter stability testing (+-10%), asset holdout validation,
...
def run_parameter_stability_test(
    ...
    """
    Evaluates strategy performance across neighboring parameter variations (0.90, 1.00, 1.10).
    Verifies edge is stable across parameter neighborhoods rather than an isolated backtest peak.
    """
    if param_mults is None:
        param_mults = [0.90, 1.00, 1.10]
    ...
    is_stable = (positive_count == len(param_mults))
...
def run_friction_and_execution_stress_test(
    ...
    # Slippage stress 10bps per side
