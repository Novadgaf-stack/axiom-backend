"""
Position Sizing Module for NEXUS-7 Research V36
Evaluates stop-distance position sizing across risk tiers (0.25%, 0.50%, 0.75%, 1.00%).
Compares Fixed Risk vs Volatility-Adjusted vs Risk-Parity vs Confidence-Weighted sizing models.
"""
...
def evaluate_position_sizing_and_growth(
    ...
    risk_tiers: List[float] = [0.0025, 0.0050, 0.0075, 0.0100]
) -> Dict[str, Any]:
    """
    Evaluates portfolio capital growth across multiple risk-per-trade tiers.
    """
    if not trades:
        return {f"risk_{int(tier*10000)}bps": {"final_balance": initial_balance, "monthly_return_pct": 0.0, "max_drawdown": 0.0, "calmar_ratio": 0.0} for tier in risk_tiers}
