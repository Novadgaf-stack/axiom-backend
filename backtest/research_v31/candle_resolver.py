"""
NEXUS-7 Research V31 — Zero-Stub Candle Traversal Resolver Module
Derives trade outcomes strictly from bar-by-bar candle traversal.
Handles execution delay, fee deduction, slippage, same-candle SL/TP collisions,
stop-distance position sizing, portfolio daily/weekly circuit breakers, asset correlation limits,
and consecutive-loss protection step-downs (5x -> 0.25%, 8x -> pause).
"""
...
def resolve_zero_stub_trades(
    df: pd.DataFrame,
    risk_fraction: float = 0.0050,      # 0.50% default research risk
    max_risk_fraction: float = 0.0075,  # 0.75% max risk cap
    fee_rate: float = 0.0015,          # 0.15% roundtrip baseline fee
    slippage: float = 0.0005,          # 0.05% per side baseline slippage
    execution_delay: int = 1,          # 1-bar execution delay
    missed_signal_pct: float = 0.0,    # 0% - 20% missed signal simulation
    ...
    while i < n - 1:
        ...
        # Track weekly circuit breaker (ISO week)
        year_week = (ts_dt.year, ts_dt.isocalendar().week)
        ...
        # Check consecutive loss protections
        ...
        if signal != 0 and not circuit_breaker_tripped and not weekly_breaker_tripped and not paused_for_review:
            if missed_signal_pct > 0 and rng.random() < missed_signal_pct:
                i += 1
                continue
            ...
            # Consecutive loss step-down: After 5 consecutive losses, reduce risk to 0.25%
            current_risk_fraction = min(risk_fraction, max_risk_fraction)
            if consecutive_losses >= 5:
                current_risk_fraction = min(current_risk_fraction, 0.0025)

            adjusted_risk_fraction = current_risk_fraction * correlation_penalty_mult
            ...
            # Traversal loop through future candles
