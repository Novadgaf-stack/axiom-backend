"""
NEXUS-7 Research V31 — Monte Carlo Simulation Module
Shuffles realized trade PnLs over 2,000 iterations to derive drawdown distributions,
losing streak distributions, risk-of-ruin estimates, and return percentiles.
"""
...
def run_monte_carlo_resampling(
    ...
    """
    Shuffles realized trade PnLs over 2,000 iterations while preserving the return distribution.
    ...
    risk_of_ruin = float(np.mean(max_dds > 0.50) * 100.0)

    return {
        "iterations": iterations,
        "median_return_pct": round(float(np.median(returns_pct) * 100), 2),
        "pct_5th_return": round(float(np.percentile(returns_pct, 5) * 100), 2),
        "pct_95th_return": round(float(np.percentile(returns_pct, 95) * 100), 2),
