"""
NEXUS-7 — RESEARCH V16 SOL/USDT 1H DEDICATED HYPOTHESIS VALIDATION PIPELINE
Performs sample expansion, market regime decomposition, parameter stability audits,
Monte Carlo bootstrap confidence intervals, and random baseline attribution testing.
"""
import asyncio
import csv
import os
...
async def run_single_sol_validation(
    candles_is: List,
    candles_oos: List,
    mode: str = "ai_mirror",
    ...
    exp_name: str = "SOL_1h_Validation",
    sample_days: int = 365,
    ...
    # 3. Monte Carlo Bootstrap Resampling (1,000 iterations)
...
def run_full_v16_validation_pipeline(days: int = 365, seed: int = 42) -> Dict:
    t0 = time.time()
    ...
    # Exp 1: Baseline Extended SOL 1h Validation
    res_base = asyncio.run(run_single_sol_validation(is_c, oos_c, mode="ai_mirror", min_confidence=88, min_adx=20.0, exp_name="1_Extended_Baseline_88", sample_days=days))
    results.append(res_base)

    # Exp 2: Parameter Stability Sweep (Confidence 82, 85, 90)
    for conf in [82, 85, 90]:
        res_conf = asyncio.run(run_single_sol_validation(is_c, oos_c, mode="ai_mirror", min_confidence=conf, min_adx=20.0, exp_name=f"2_Param_Conf_{conf}", sample_days=days))
        results.append(res_conf)

    # Exp 3: ADX Stability Sweep (15.0, 25.0)
    for adx_val in [15.0, 25.0]:
        res_adx = asyncio.run(run_single_sol_validation(is_c, oos_c, mode="ai_mirror", min_confidence=88, min_adx=adx_val, exp_name=f"3_Param_ADX_{adx_val}", sample_days=days))
        results.append(res_adx)

    # Exp 4: Random Entry Baseline Attribution Test
    res_rand = asyncio.run(run_single_sol_validation(is_c, oos_c, mode="technical_only", min_confidence=88, min_adx=20.0, exp_name="4_Random_Attribution_Baseline", sample_days=days))
    ...
    # Generate research_v16_sol_1h_validation_report.md
    report_lines = [
        ...
        f"**DATA SAMPLE:** {days} Days (~8,760 1h Candles) — 70% In-Sample / 30% Out-of-Sample Holdout  ",
        f"**TRANSACTION COSTS:** Binance Spot Taker Fee 0.10% + Slippage 0.05%  ",
        f"**MONTE CARLO BOOTSTRAP:** 1,000 Resampling Iterations  ",
    ...
    for r in results:
        report_lines.append(
            f"| `{r.experiment_name}` | {r.total_trades} | {r.win_rate_pct:.1f}% | {r.net_pf:.2f} | ${r.net_expectancy_usd:.2f} | **{r.net_expectancy_r:+.2f}R** | [{r.bootstrap_pf_ci_low:.2f}, {r.bootstrap_pf_ci_high:.2f}] | {r.walk_forward_efficiency_pct:.1f}% | **{r.verdict}** |"
        )
    ...
    with open("./strategy_research/research_v16_sol_1h_validation_report.md", "w", encoding="utf-8") as f:
        f.write(report_md)

    art_dir = r"C:\Users\Administrator\.gemini\antigravity\brain\d5901043-efa6-433c-b61e-0438599312b6"
    if os.path.exists(art_dir):
        with open(os.path.join(art_dir, "research_v16_sol_1h_validation_report.md"), "w", encoding="utf-8") as f:
            f.write(report_md)

    csv_path = "./strategy_research/v16_sol_validation_summary.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "experiment_name", "symbol", "timeframe", "sample_days", "total_trades",
            "win_rate_pct", "net_pf", "net_expectancy_usd", "net_expectancy_r",
            "bootstrap_pf_ci_low", "bootstrap_pf_ci_high", "walk_forward_efficiency_pct", "verdict"
        ])
        for r in results:
            writer.writerow([
                r.experiment_name, r.symbol, r.timeframe, r.sample_days, r.total_trades,
                r.win_rate_pct, r.net_pf, r.net_expectancy_usd, r.net_expectancy_r,
                r.bootstrap_pf_ci_low, r.bootstrap_pf_ci_high, r.walk_forward_efficiency_pct, r.verdict
            ])
...
if __name__ == "__main__":
    run_full_v16_validation_pipeline()
