"""
Statistical Evaluator Module for NEXUS-7 Research V39
...
def compute_trade_statistics_v39(
...
def evaluate_v39_promotion_gates(
    ...
    """
    Evaluates strict V39 machine-readable promotion gates for ROBUST_PROFITABLE status.
    """
    if not is_real_data:
        return "REAL_DATA_REQUIRED", {"gate_real_data": False}
    ...
    wf_ratio = wf_positive_windows / max(1, total_wf_windows)

    gates = {
        "gate_real_data": bool(is_real_data),
        "gate_pf_min": bool(pf >= 1.15),
        "gate_exp_positive": bool(exp > 0.0),
        "gate_bootstrap_lower": bool(bs_lower > 1.00),
        "gate_walk_forward": bool(wf_ratio >= 0.75),
        ...
        "gate_asset_replication": bool(asset_replication_passed),
        "gate_mc_drawdown": bool(mc_95_dd_pct <= 25.0),
        "gate_multiple_testing": bool(dsr_passed),
    ...
    if is_fragile:
        ...
        verdict = "V39_PROFITABLE_BUT_NOT_ROBUST"
    elif tpd >= 0.75 and (pf <= 1.0 or exp <= 0.0):
        verdict = "V39_FREQUENT_BUT_UNPROFITABLE"
    elif stats.get("total_trades", 0) < 10:
        verdict = "V39_INSUFFICIENT_SAMPLE"
    else:
